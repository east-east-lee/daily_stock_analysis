...
- [文档] 对应 Issue #1497：本次变更仅做文档澄清，不包含 `SCHEDULE_TIME` 的运行时控制能力实现；`SCHEDULE_TIME` 仍仅影响本地内置调度（`python main.py --schedule`）。GitHub Actions 的触发时刻继续由 `00-daily-analysis.yml` 的固定 `cron` 决定（周一到周五 10:00 UTC / 北京18:00）；如需调整请修改 workflow cron。Issue #1497 的 GitHub Actions 实时执行能力仍待办。
