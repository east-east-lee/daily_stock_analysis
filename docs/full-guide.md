...
`AI 建议池 / 决策信号` 规划把现有报告和 Agent 输出中的操作建议、价格计划、风险理由、置信度和证据摘要沉淀为 `DecisionSignal` / `AISuggestion` 结构化资产。P0 仅定义语义契约，不新增配置项、API、数据库表或 Web 页面，也不改变现有报告展示、通知格式、回测、告警和持仓流程。

字段、动作枚举、市场阶段、horizon、`plan_quality`、生命周期、脱敏边界和回滚说明见 [AI 建议池 / 决策信号语义契约](decision-signals.md)。后续实现必须保持建议生成失败不影响主分析流程，缺少入场价、止损或目标价时只降低 `plan_quality`，不得编造价格计划。
